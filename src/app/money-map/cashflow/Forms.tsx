...
import { createFixedExpense, createCommitment, createInsurance } from "./actions";
  ...
  return (
    <form
      ...
      action={(fd) => {
        setErr(null);
        start(async () => {
          const r = await createInsurance(fd);
          if (r && "error" in r && r.error) setErr(r.error);
          else (document.getElementById("ins-form") as HTMLFormElement)?.reset();
        });
      }}
      className="grid grid-cols-1 md:grid-cols-6 gap-2 mb-4 text-[11px]"
    >
      <TextInput name="name" placeholder="POLICY (AIA Life, etc)" required className="uppercase md:col-span-2" />
      <NumInput name="amount" placeholder="PREMIUM" required />
      <Select name="currency" options={CURRENCIES} defaultValue="SGD" uppercase={false} />
      <Select name="for_who" options={FOR_WHO} defaultValue="self" />
      <div className="flex gap-2">
        <Select name="cycle" options={CYCLES} defaultValue="monthly" className="flex-1" />
        <SubmitBtn pending={pending} />
      </div>
