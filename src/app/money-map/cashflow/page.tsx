...
import { FixedExpenseForm, CommitmentForm, InsuranceForm } from "./Forms";
import { FxRow, IcRow, InsRow } from "./Rows";
...
export default async function Page() {
  ...
  const [fxRows, icRows, insRows] = await Promise.all([
    ...
    db.select().from(insurance_policies).where(eq(insurance_policies.user_id, user.id)).orderBy(asc(insurance_policies.name)),
  ]);
  ...
  const insEnriched = await Promise.all(
    insRows.map(async (r) => ({
      id: r.id, name: r.name, amount: r.amount, currency: r.currency, cycle: r.cycle, for_who: r.for_who,
      monthly: await convert(toMonthly(Number(r.amount), r.cycle), r.currency, base),
    }))
  );
  ...
  const insTotal = insEnriched.reduce((s, r) => s + r.monthly, 0);

  return (
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-3 [&>*:nth-child(3)]:lg:col-span-2">
      ...
      <Panel title="INSURANCE" meta={`${insRows.length} POLICIES · ${fmtMoney(insTotal, base, 0)}/MO`}>
        <InsuranceForm />
        <table className="w-full text-[11px] border-collapse">
          <thead>
            <tr className="text-muted uppercase tracking-[0.5px]">
              <th className="text-left py-1 border-b border-border font-normal">POLICY</th>
              <th className="text-left py-1 border-b border-border font-normal pl-3">FOR WHO</th>
              <th className="text-right py-1 border-b border-border font-normal">PREMIUM</th>
              <th className="text-left py-1 border-b border-border font-normal pl-3">CYCLE</th>
              <th className="text-right py-1 border-b border-border font-normal pl-3">MONTHLY ({base})</th>
              <th className="text-right py-1 border-b border-border font-normal pl-3 w-20">ACTIONS</th>
            </tr>
          ...
          <tbody>
            {insEnriched.length === 0 && (
              <tr><td colSpan={6} className="text-muted text-center py-4 italic">no insurance policies yet</td></tr>
            )}
            {insEnriched.map((r) => <InsRow key={r.id} r={r} base={base} />)}
          </tbody>
        </table>
      </Panel>
    </div>
